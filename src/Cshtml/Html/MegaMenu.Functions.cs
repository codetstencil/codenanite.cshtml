    ...
    public partial class MegaMenu
    {
        ...
        private string MenuLinks()
        {
            ...
            foreach (var list in lists)
            {
                ...
                {
                    //text += MenuLink(item.TableName, Pluralize(item.TableName)+"/Index")).Li("class", "nav-link text-dark").AddCarriage();
                    var menuItem = Pluralize(item.TableName, PreserveTableName());
                    text += MenuLink(item.TableName, menuItem+"/Index")
