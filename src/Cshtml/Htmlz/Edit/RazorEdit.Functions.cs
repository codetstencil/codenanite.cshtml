...
{
    public partial class RazorEdit : ExpansionBase
    {
            ...
            BuildText("class", "row", "div", 0);
            {
                BuildText("class", "col-md-4", "div", 4);
                {
                    BuildText("method", "post", "form", 8);
                    {
                        ...
                        var input = GetHtmlString("type", "hidden", "asp-for", GetTableAndPrimaryKey(_table));
                        AppendText(Indent(12) + input.Tag("input", "/"));
        ...
        private string GetFormGroups(List<ISchemaItem> columns, int indent)
        {
            BuildSnippet(null);
            ...
            BuildSnippet(GetHtmlString("class", "form-group", "div"), indent);
            {
                var form = GetHtmlString("type", "submit", "value", "Save") + " " + GetHtmlString("class", "btn btn-primary");
                BuildSnippet(form.Tag("input", "/"), indent + 4);
                BuildSnippet(GetHtmlString("asp-page", "./Index", "class","btn btn-success").Tag("a") + "Back to List".TagEnd("a"), indent + 4);   //***
        ...
        private void GetLookupTag(ISchemaItem column, int indent)
        {
            ...
            var lookup = "<select " + GetHtmlString("asp-for", column.TableName + "." + column.ColumnName, "class", "form-control",
                "asp-items",
                "@Model." + selectList) + ">";
            //var option = ("value=" + "".AddQuotes() + ">-- Select " + _table + " --").Tag("option").TagEnd("option");
